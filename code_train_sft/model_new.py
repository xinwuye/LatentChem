...
class Qwen3MoleculeLLM(PreTrainedModel):
    def __init__(self, 
        ...
        if vllm:
            ...
            # Initialize vLLM instance for faster inference
            from vllm import LLM
            self.vllm = LLM(model=qwen_model_name, dtype="bfloat16", enable_prompt_embeds=True)
            self.model = None
    ...
    def generate_vllm(
        ...
        **kwargs,
        ...
        # Due to vLLM internal assertion issues when using prompt_embeds with n > 1,
        # we handle multiple samples by making separate individual calls
        vllm_outputs = []
        for sample_fused in fused_samples_list:
            prompts = [{"prompt_embeds": sample_fused}]
            sampling_params = SamplingParams(
                temperature = temperature,
                top_p = top_p,
                max_tokens = max_new_tokens,
                n = sample_count  # This will work for individual calls
            )
            output = self.vllm.generate(prompts, sampling_params, **kwargs)
            vllm_outputs.append(output[0])  # Take the output for this specific sample
        
        # Return the outputs in the expected format regardless of sample_count
        return [
            [
                sample_output.text
                for sample_output in request_outputs.outputs
            ]
    ...
    def cleanup_vllm(self):
        """Clean up the vLLM instance to free GPU memory"""
        if hasattr(self, 'vllm') and self.vllm is not None:
            del self.vllm
            self.vllm = None
