...
class BaseTaskEvaluator(ABC):
    ...
    def _load_samples(self, log_dir: str, model_name: str) -> List[dict]:
        ...
        # Check if the regular log file exists
        if os.path.exists(path):
            with open(path, 'r') as f:
                return json.load(f)
        else:
            # If not found, try to load from a merge.json file
            merge_path = os.path.join(log_dir, "merge.json")
            if os.path.exists(merge_path):
                with open(merge_path, 'r') as f:
                    data = json.load(f)
                    # Extract test_results if it exists
                    if 'test_results' in data:
                        all_results = data['test_results']
                    else:
                        all_results = data

                    # Filter results by task name (using the task_name from the calling context)
                    # We need to get the task_name from the calling context
                    # For now, we'll return all results and filtering will happen elsewhere if needed
                    return all_results
            else:
                raise ValueError(f"log file not found: {path} or {merge_path}")
    ...
    def _load_gt_raw(self, gt_path: str, taskname: str) -> List[Any]:
        ...
        if not os.path.exists(path):
            ...
            if taskname!="ligand" and taskname!="reactant" and taskname!="solvent":
