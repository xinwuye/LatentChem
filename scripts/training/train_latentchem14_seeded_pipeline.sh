...
if [[ ! "$vllm_seed" =~ ^[0-9]+$ ]]; then
  echo "vLLM seed must be an integer, got: $vllm_seed" >&2
  exit 1
fi
...
stage1_dir="./outputs/latentchem14-stage1/stage1"
stage4_output="./outputs/latentchem14-stage4-correct"
lora_path="${stage1_dir}/lora_weights"
projector_path="${stage1_dir}/mm_projector.pt"

if [[ ! -d "${lora_path}" ]]; then
  echo "Missing latentchem14 stage1 LoRA directory: ${lora_path}" >&2
  exit 1
fi
if [[ ! -f "${projector_path}" ]]; then
  echo "Missing latentchem14 stage1 projector checkpoint: ${projector_path}" >&2
  exit 1
fi
if [[ -e "${stage4_output}" ]]; then
  echo "Refusing to reuse existing stage4 output path: ${stage4_output}" >&2
  exit 1
fi

echo "Starting latentchem14 stage4 rerun with seed=${seed}, vllm_seed=${vllm_seed}..."
