...
data = []
with open('/zengdaojian/zhangjia/BioLatent/Bio-LatentCOT/eval/silmilarity_matrix/sorted_data_qed.jsonl', 'r', encoding='utf-8') as f:
    for line in f:
        line = line.strip()
        if line:  # 跳过空行
...
latent_blocks = [item['latent_block'][num] for item in data]
...
# 转换为 NumPy 矩阵
matrix = np.vstack(latent_blocks)
# 计算余弦相似度矩阵
...
df.to_csv('/zengdaojian/zhangjia/BioLatent/Bio-LatentCOT/eval/silmilarity_matrix/bio_latent_similarity_matrix_'+str(num)+'.csv')

print("✅ 相似度矩阵已保存到 bio_latent_similarity_matrix_"+str(num)+".csv")